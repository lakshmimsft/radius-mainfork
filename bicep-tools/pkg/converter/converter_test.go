...
func TestAddSchemaType_AdditionalPropertiesAny_Error(t *testing.T) {
	...
	if !strings.Contains(err.Error(), expectedError) {
		g := err.Error()
		t.Errorf("Expected error to contain '%s', got '%s'", expectedError, g)
	}
}

func TestAddSchemaType_PlatformOptionsAllowsAny(t *testing.T) {
	schema := &manifest.Schema{
		Type: "object",
		Properties: map[string]manifest.Schema{
			"platformOptions": {
	...
	result, err := addSchemaType(schema, "testPlatformOptionsAny", typeFactory)
	if err != nil {
		t.Fatalf("Expected no error, got: %v", err)
	}
	...
	rootObject, ok := allTypes[typeRef.Ref].(*types.ObjectType)
	if !ok {
		t.Fatal("Expected root type to be an ObjectType")
	}

	platformOptionsProp, exists := rootObject.Properties["platformOptions"]
	if !exists {
		t.Fatal("Expected platformOptions property to exist")
	}

	platformOptionsRef, ok := platformOptionsProp.Type.(types.TypeReference)
	if !ok {
		t.Fatal("Expected platformOptions type to be a TypeReference")
	}
	platformOptionsObj, ok := allTypes[platformOptionsRef.Ref].(*types.ObjectType)
	if !ok {
		t.Fatal("Expected platformOptions to be an ObjectType")
	}

	if platformOptionsObj.AdditionalProperties == nil {
		t.Fatal("Expected platformOptions additionalProperties to be defined")
	}

	additionalPropsRef, ok := platformOptionsObj.AdditionalProperties.(types.TypeReference)
	if !ok {
		t.Fatal("Expected additionalProperties to be a TypeReference")
	}
	if _, ok := allTypes[additionalPropsRef.Ref].(*types.AnyType); !ok {
		t.Fatal("Expected additionalProperties to be an AnyType")
	}
}
